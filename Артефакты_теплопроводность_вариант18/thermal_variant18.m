% Расчет стационарной теплопроводности для варианта 18.
...
variants = 18;
insulatedBoundaries = 2;
folders = {'matlab_results_variant18'};
...
disp('Расчет завершен. Результаты сохранены в папке matlab_results_variant18.');
...
function [points, labels] = boundaryPoints(variantNumber, R, nArc, nSeg)
    ...
    if variantNumber ~= 18
        error('Этот файл предназначен только для варианта 18.');
    end

    [points, labels] = addArc(points, labels, R, pi, pi/2, 1, nArc, false);
    [points, labels] = addArc(points, labels, R, pi/2, 0, 2, nArc, true);
    [points, labels] = addSegment(points, labels, [R, 0], [0, 0], 3, nSeg, true);
    [points, labels] = addSegment(points, labels, [0, 0], [R/sqrt(2), -R/sqrt(2)], 4, nSeg, true);
    [points, labels] = addArc(points, labels, R, -pi/4, -pi/2, 5, nArc, true);
    [points, labels] = addArc(points, labels, R, -pi/2, -pi, 6, nArc, true);
end
...
function [points, triangles, boundaryNodes, processorNodes, processor, polygon, centroid] = ...
    ...
    [polygon, ~] = boundaryPoints(variantNumber, R, 80, 45);
